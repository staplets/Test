 ...
 * Date: 08/09/14
 * Solution description: File holds the implementation of the to do list functions.
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
{
    //Create new pointer                                                                                //Worked On /* FIXME */
    Task *newPtr;
    newPtr = malloc(sizeof(Task));
    assert(newPtr != 0);

    //assign input to new Task
    strcpy(newPtr->description, desc);
...
void printList(DynArr *heap)
{
    ...
    int k;
    Task *task;
    int halfSize = sizeDynArr(heap)/2;
    int size = sizeDynArr(heap) ;                                                                  //Worked On  /* FIXME: Write this */
