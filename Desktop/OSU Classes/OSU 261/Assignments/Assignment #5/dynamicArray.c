 ...
 * Date: 08/09/14
 * Solution description: File holds the implementation of the binary search tree functions.
 */
...
{
                                                                                                           //Worked On     /* FIXME */
    assert(DynArr(heap) > 0);
    return DynArr(heap, 0);
...
void addHeap(DynArr *heap, TYPE val)
{
	...
	int pos = sizeDyArray(heap);
	addDynArr(heap, val); /* adds to end – now need to adjust position */

	while(pos != 0)
	{
		parent = (pos-1)/2;
		if(compare(getDyArray(heap, pos), getDyArray(heap, parent)) == -1)
		{
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
    ...
    {
    /*1. it has 2 children.
    /* 2. Get Index of smallest child */
    int smallest = _smallerIndexHeap(heap, leftIdx, rightIdx);
    /* 3. Compare smallest child to pos */
    if (smallest < pos)
        {
            /*4.  If necessary, swap and call _adjustHeap(max, minIdx). */
    ...
    */
    ...
    {
        /*1. it has only 1 child*/
        /*  2. Compare child to parent */
    if(leftIdx < pos)
        {
                 /* 3. If necessary, swap and call _adjustHeap(max, leftidx) */
                swap(heap, pos, leftidx);
...
void removeMinHeap(DynArr *heap)
{
    ...
    //Take last item and place at first index
...
{
     //assign position and max for building                                                                                                                      //Worked On     /* FIXME */
    int size = sizeDynArr(heap);
    int position = size/2 - 1;

    //adjust heap from non leaf nodes
    while(position >= 0)
    {
        _adjustHeap(heap, size, position);
...
{
                                                                                                    //Worked On /*FIX ME*/
    int sortSize = sizeDynArr(heap);

      //Build Heap
      _buildHeap(heap);

      //repeat swap and adjust while decrementing last each time
      while(sortSize >= 0)
      {
           //Swap 1st and last elements
          swapDynArr(heap,0, sortSize);

          //adjust heap from 0 to last.. not inclusive
          _adjustHeap(heap,sortSize, 0);
