...
{
     int i;
    //struct cirListDeque *stackSearch = NULL;
             ...
             {//printf("test");
                ...
                {       /*see if neighbor is adjacent*/
                    if(!(reached->neighbors[i]->isVisited))
                    {
                        ...
                        /*add source to stack*/ //printf("test");
             ...
             {//printf("test");
                ...
                {       /*see if neighbor is adjacent*/
                    if(!(reached->neighbors[i]->isVisited))
                    {
                        ...
                        /*add source to stack*/ //printf("test");
                        addBackCirListDeque(stackSearch, reached->neighbors[i]);
                    }
...
int main ()
{
	...
	for(i = 0; i < g.numVertices; ++i)
		for(j = i+1; j < g.numVertices; ++j)
		{
			...
			if(useDFS)
				if(DFS(&g, &g.vertexSet[i], &g.vertexSet[j]))   //DFSRecursive(&g, &g.vertexSet[i], &g.vertexSet[j]))
					printf("reachable!\n");
				else
					printf("***UNREACHABLE***\n");
			else
