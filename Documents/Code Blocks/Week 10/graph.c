...
{
   int i;
    /*create and initialize stack*/    struct cirListDeque *stackSearch = malloc(sizeof(struct cirListDeque));
    stackSearch->size = 0;
    ...
    while(!isEmptyCirListDeque(stackSearch))
    {
        ...
        if(reached == destination)
        {
            ...
            return 1;
             ...
             for(i = 0; i < reached->numNeighbors; ++i)
             {
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    /*Create and initialize stack*/
    struct cirListDeque *queueSearch = malloc(sizeof(struct cirListDeque));
    queueSearch->size = 0;
    initCirListDeque(queueSearch);
                                        //Worked on */   /* FIXME you will write this */
    clearVisited(g);

      /*add source to stack*/
    addFrontCirListDeque(queueSearch, source);

    /*while stack is not empty*/
    while(!isEmptyCirListDeque(queueSearch))
    {
        struct Vertex *reached;

        /*get value from front of stack*/
        reached = frontCirListDeque(queueSearch);

        /*Remove value from front of stack*/
        removeFrontCirListDeque(queueSearch);
        //printf("%c", reached->label);
        //if not visited
        if(!(reached->isVisited))
        {
        ...
        if(reached == destination)
        {
            ...
            removeAllCirListDeque(queueSearch);
            return 1;
        }
             ...
             {//printf("test");
                ...
                {       /*see if neighbor is adjacent*/
                    if(!(reached->neighbors[i]->isVisited))
                    {
                        ...
                        /*add reached to queue*/ //printf("test");
                        addBackCirListDeque(queueSearch, reached->neighbors[i]);
                    }
